...
use App\Mail\OtpVerificationCodeEmail;
...
class OTPService implements OTP
{
    ...
    {
        Mail::to($address)->send(new OtpVerificationCodeEmail($code, $address));
    }
    ...
    {
        $query = $this->repo->findAll(returnResults: false)
            ->where('id', $refId)
            ->where('code', $code)
            ...
            ->where('created_at', '>=', now()->subMinutes($this->expireInMin)->format('Y-m-d H:i:s'));
    ...
    {
        Mail::to($notification->notifiable->email)->send(new OtpVerificationCodeEmail($notification->code, $notification->notifiable->email));
    }
}
